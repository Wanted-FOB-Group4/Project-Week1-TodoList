...
class SwipeableItem extends PureComponent {
  ...
  handleDragStart = ({ clientX, clientY }) => {
    ...
    this.listElement.className = 'swipeableListItemContent'
    if (this.contentLeft !== null) {
      this.contentLeft.className = 'swipeableListItemContentLeft'
    }

    if (this.contentRight !== null) {
      this.contentRight.className = 'swipeableListItemContentRight'
    }
  ...
  playReturnAnimation = () => {
    ...
    if (listElement) {
      listElement.className = 'swipeableListItemContent swipeableListItemContentReturn'
    ...
    if (contentLeft !== null) {
      ...
      contentLeft.className = 'swipeableListItemContentLeft swipeableListItemContentLeftReturn'
    }

    if (contentRight !== null) {
      contentRight.style.opacity = 0
      contentRight.className = 'swipeableListItemContent-right swipeableListItemContent-rightReturn'
    }
  }
  ...
  playRemoveAnimation = (direction) => {
    ...
    if (listElement) {
      listElement.className = 'swipeableListItemContent swipeableListItemContentRemove'
    ...
    if (this.isSwiping()) {
      if (listElement) {
        if (left < listElement.offsetWidth * threshold * -1) {
          this.playActionAnimation(swipeLeft.actionAnimation, DragDirection.LEFT)
          this.handleSwipedLeft()
          actionTriggered = true
        } else if (left > listElement.offsetWidth * threshold) {
          this.playActionAnimation(swipeRight.actionAnimation, DragDirection.RIGHT)
  ...
  setDragDirection = (x, y) => {
    if (this.dragDirection === DragDirection.UNKNOWN) {
      ...
      switch (octant) {
        case 0:
          if (this.contentRight !== null && horizontalDistance > this.dragHorizontalDirectionThreshold) {
        ...
        case 3:
          ...
          break
        case 4:
          if (this.contentLeft !== null && horizontalDistance > this.dragHorizontalDirectionThreshold) {
  ...
  isSwiping = () => {
    ...
    const horizontalDrag = this.dragDirection === DragDirection.LEFT || this.dragDirection === DragDirection.RIGHT
    ...
    if (elapsed > FPS_INTERVAL && this.isSwiping()) {
      ...
      if (this.props.onSwipeProgress && this.listElement) {
        ...
        if (listElementWidth !== 0) {
          const swipeDistance = Math.max(0, listElementWidth - Math.abs(this.left))
      ...
      if (opacity < 1 && opacity.toString() !== contentToShow.style.opacity) {
        ...
        const contentToHide = this.left < 0 ? this.contentRight : this.contentLeft
  ...
  render() {
    ...
    return (
      <div className='swipeableListItem' ref={this.bindWrapper}>
        {swipeLeft && (
          <div className='swipeableListItemContentLeft' data-testid='swipeLeftContent' ref={this.bindContentLeft}>
            {swipeLeft.content}
          </div>
        )}
        {swipeRight && (
          <div className='swipeableListItemContentRight' data-testid='swipeRightContent' ref={this.bindContentRight}>
            {swipeRight.content}
          </div>
        )}
        <div className='swipeableListItemContent' data-testid='content' ref={this.bindListElement}>
          {children}
        </div>
      </div>
    )
